...
from intent.api import get_intent
from simbert.simbert_base import generateSimSentence
import time
# from ner.api import get_entity
...
class AnswerByOwnthink(object):
    def __init__(self,):
        ...
        self.stopFile = dir_path + '/data/stopwords.txt'    # 加载停用词文件
        self.attrFile = dir_path + '/data/attr_mapping.txt'    # 映射文件
        self.fuzzyFile = dir_path + '/data/fuzzy.txt'        # 不完全实体的映射匹配
    ...
    def get_entity(self, sent: str):
        entities = self.NER.recognize(sent)
        # print('api.py entities:', entities)
        return entities

    def searchQuestion(self,question:str):
        # 获取实体和属性识别结果，根据SENT、OENT排序
        entity_dict,self.prop_dict = self.getEntProp2dict(question)

        # 判断问句类别
        # with graph.as_default():
        #     question_type = self.quesClass(question)  # ['SP_O','SPP_O','SO_P','PO_S','OP_S']，进行分类
        question_type = 0 # 暂时写定为spo
        ...
        if question_type == 0:
            ...
            # print('333333333333333',entity)
            if prop_len == 1:
                prop = list(self.prop_dict.keys())[0]
                # print('6666666666666', entity, prop)
                # 识别出 S P,求O
    ...
    def SPO(self,entity,prop):
        print('搜索类型：SPO')
        # 直接通过实体名+属性值查询
        res = list((self.query.SP_O(entity,prop)).values())
        ans = []
        if res: # SP直接成功查得O
            # print(res)
            ans.append(','.join(res))
            ...
            # 获取与实体相关的歧义关系列表[{ }]
            data_list_dict = self.query.Q_Z_ByEntity(entity)
            if data_list_dict:     # 歧义关系存在的情况
                # print('dddd',data_list_dict)
                ans = ['none']                      # 要返回的列表结果
    ...
    def O_By_SP(self,entity):
        ...
        if data_list_dict:                      # 歧义关系存在的情况
            ...
            # print('8888888888888', S_P_list, rela_info, self.ques)
            if rela_info:
                ques = [self.ques]  # 将原问句先加入ques列表
                ...
                temp_sim_val = max(sim_val_list)
                # print('sssssssimmmmmm', sim_val_list, temp_sim_val)
                comp_sim_val ='%.4f'%(float(temp_sim_val))
                if float(comp_sim_val) > float(sim_val):
                    r_name_index = sim_val_list.index(temp_sim_val)-1
                    # print('rrrrrrrr',r_name_index,prop_list)
                    r_name = prop_list[r_name_index]
    ...
    def P_By_SO(self,entity_dict:dict):
        print('搜索类型：P_SO')
        entity_01,entity_02 = list(entity_dict.keys())[0],list(entity_dict.keys())[1]
    ...
    def getEntProp2dict(self,question):
        ...
        with graph.as_default():
            # print('0000000000000')
            ans = self.get_entity(question)
            # print('0000000000, ans', ans)
        if not ans:
            ans = self.get_entity(question + '?')
        #     print('11111111111, ans', ans)
        # print('222222222, ans', ans)
        ...
        if ans:
            for val in ans:
                if val[1] == 'SUB' or val[1] == 'OBJ':
                    ent[val[0]]=val[1]
                elif val[1] == 'PRO':
                    pos = question.find(val[0])
                    prop[val[0]] = pos
        print('55555555555', ent, prop)
        return ent,prop
...
class GiveFlaskWebData(object):
    ...
    def getWebTypeData(self, question: str):
        ...
        link = []
        start_time = time.time()
        ans, searchPath, sim, question_type = self.qa.searchQuestion(question)
        # print('9999999999', ans, searchPath, sim, question_type)
        end_time = time.time()
        timeUsed = str(int(float('%.3f' % (end_time - start_time)) * 1000)) + 'ms'
        ques_type_list =  ['SP->O','SPP->O','PO->S','OP->S','SO->P']
        ...
        if question_type == 'SP->O':
            ...
            # print('11111111111', ent_attr_list, res_list)
            idx = 1
            temp_dict = {
                # 'id':idx,
